...
public class Camera2BasicFragment extends Fragment
    ...
    private void setUpCameraOutputs(int width, int height, int facingId) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                // 원래는 안쓰는 코드였지만 이제부터 사용한다
                // 전면 후면 관리하는 코드
                Integer facing = characteristics.get(CameraCharacteristics.LENS_FACING);
                //CameraCharacteristics.LENS_FACING_FRONT아래 facingId에서 대체된 값
    ...
    private void openCamera(int width, int height) {
        ...
        // 카메라 전 후방 변경하기 3
        setUpCameraOutputs(width, height,facingId ); //CameraCharacteristics.LENS_FACING_FRONT 초기값
        configureTransform(width, height);
        Activity activity = getActivity();
        CameraManager manager = (CameraManager) activity.getSystemService(Context.CAMERA_SERVICE);
        try {
